...
void encode_player(uint8_t **array_player){ // change size 
	*array_player = calloc(2,sizeof(uint8_t));
...
void encode_enemies(uint8_t **array_enemies){ // change size key
	...
	array_enemies = calloc(smallest_length <<2, sizeof(uint8_t));
	uint8_t length_array_mask = 0b00011111;
	for(uint8_t i = 0; i < smallest_length; i++){
		...
		byte_2 = game.enemies_array[i].x;
		byte_2 = byte_2 << 3;
		byte_2 = game.enemies_array[i].y;
...
void encode_game(){
	...
	uint8_t array_total[32] = array_1;
...
void decode_enemies(uint8_t *array){ 
	...
	uint8_t array[MAX_AMOUNT_ENEMIES];  
